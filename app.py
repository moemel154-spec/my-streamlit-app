...
/* UI elements */
.section-header { font-size:17px; font-weight:700; color: var(--text-dark); margin: 4px 0 6px 0 !important; }
.card { background: var(--card); padding: 12px 14px; border-radius: 10px; box-shadow: 0 3px 10px rgba(7,57,42,0.05); border-left: 3px solid rgba(123,211,176,0.55); margin-bottom: 10px; animation: floatUp 0.35s ease backwards; }
.stTextInput>div>div>input { padding: 8px !important; font-size: 15px !important; border-radius: 8px !important; border: 1px solid #cfeee2 !important; }
mark { background-color: var(--mark); padding: 3px 5px; border-radius: 6px; font-weight:700; }
.warning-pill { display:inline-block; background: linear-gradient(90deg, rgba(255,230,230,0.9), rgba(255,250,230,0.9)); color: #7a2618; padding: 6px 10px; border-radius: 999px; font-weight:700; border: 1px solid rgba(250,180,160,0.6); box-shadow: 0 6px 18px rgba(250,160,140,0.06); animation: pulse 2.8s infinite; }
@keyframes pulse { 0% { box-shadow: 0 0 0 0 rgba(123,211,176,0.12); } 70% { box-shadow: 0 0 18px 6px rgba(123,211,176,0.04); } 100% { box-shadow: 0 0 0 0 rgba(123,211,176,0); } }
.stButton>button { background: linear-gradient(90deg,var(--accent-start),var(--accent-end)); color: #013826; font-weight:700; border-radius:8px; padding:8px 14px; border: none; }
...
# ----------------- MODEL CONFIG -----------------
# Per user's request we explicitly use the 2.5 model name for v1beta:
MODEL_NAME = "models/gemini-2.5-flash"

# Rate control & cache
MIN_SECONDS_BETWEEN_CALLS = 1.2
_last_call_time = 0.0
_prompt_cache: Dict[str, str] = {}  # simple in-memory cache keyed by prompt-hash

# ----------------- HELPERS -----------------
def _prompt_key(prompt: str, web: bool) -> str:
    """Create a short cache key for a prompt + web flag."""
    h = hashlib.sha256((prompt + ("|web" if web else "|noweb")).encode("utf-8")).hexdigest()
    return h
...
def ask_gemini(prompt: str, web: bool = False, max_retries: int = 3) -> str:
    """
    Call Gemini model with:
    - explicit MODEL_NAME ("models/gemini-2.5-flash") as requested
    - simple cache to reduce repeated identical calls
    - exponential backoff on errors (including 429)
    - minimum delay between calls to reduce burst limits
    Returns text or an error string starting with "❌ Error:".
    """
    global _last_call_time, _prompt_cache

    if not gemini_key:
        return "❌ Error: No API key provided in sidebar."

    key = _prompt_key(prompt, web)
    ...
    while attempt < max_retries:
        try:
            ...
            err_str = str(e)
            # If it's a rate/quota error, do exponential backoff before retrying
            attempt += 1
            # For 429-like or quota messages, wait more
            sleep_seconds = backoff
            backoff *= 2
            # If last attempt, return explicit error
